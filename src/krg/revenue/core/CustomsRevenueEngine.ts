...
export class KRGCustomsRevenueEngine {
  public static evaluateCustomsRevenue(): CustomsRevenueSummary[] {
    const declarations = KRGCustomsDeclarationEngine.getAllDeclarations();
    
    return declarations.map(decl => {
      // Calculate from Tariff engine directly to get a regional rate
      ...
      const isIntegrityOk = Math.abs(calculatedDutyUSD - recordedDutyUSD) < 1;
